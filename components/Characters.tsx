...
const Characters = ({
  ...
  signal: React.RefObject<Character>;
  characters: React.RefObject<Character[]>;
  media: React.RefObject<Media[]>;
  visible: boolean;
  sorting: React.RefObject<CharacterSorting>;
  ...
  const primaryMedia = signal.current.media?.[0];

  const primaryMediaRef = primaryMedia
    ? media.current.find(({ id }) => primaryMedia.mediaId === id)
    : undefined;

  const rating = getRating({
    popularity: signal.current.popularity ?? primaryMediaRef?.popularity ?? 0,
    role:
      !signal.current.popularity && primaryMediaRef
        ? primaryMedia?.role
        : undefined,
  });
  ...
  const onAutogenerate = useCallback(() => {
    ...
    const characterName = signal.current.name.english;

    const _media = signal.current.media?.[0];

    const mediaTitle = _media
      ? media.current.find(({ id }) => _media.mediaId === id)?.title?.english
      : undefined;
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [signal.current, zeroChanModalVisible]);
      ...
      >
        <button
          ...
          onClick={() => {
            ...
            characters.current = [item, ...characters.current];
        ...
        {Object.values(characters.current).map((char, i) => {
          ...
          return (
            <div
              ...
              key={characters.current[i].id}
              onClick={() => {
                signal.current = characters.current[i];
        ...
        >
          {zeroChanModalVisible ? (
            <ZeroChanModal
              ...
              character={signal.current.name.english}
              media={primaryMediaRef?.title.english}
              callback={(imageUrl) => {
                signal.current.images = [{ url: imageUrl }];
                onCharacterUpdate();
                forceUpdate();
              }}
            />
            ...
            <>
              <div className={"flex flex-row-reverse ml-auto gap-2"}>
                ...
                <Trash2
                  ...
                  onClick={() => {
                    const i = characters.current.findIndex(
                      ({ id }) => signal.current.id === id
                    );

                    if (i > -1 && confirm(i18n("deleteCharacter"))) {
                      characters.current.splice(i, 1);
                      forceUpdate();
                      requestAnimationFrame(() => hideDialog("characters"));
                    }
                  }}
              ...
              <TextInput
                ...
                pattern=".{1,128}"
                label={i18n("name")}
                value={signal.current.name.english ?? ""}
                onInput={(value) => {
                  signal.current.name.english = value;
                  onCharacterUpdate();
                }}
              ...
              <div className={"flex flex-wrap gap-2"}>
                <Select
                  ...
                  data-warning={!signal.current.media?.length}
                  defaultValue={signal.current.media?.[0]?.mediaId}
                  list={media.current.reduce((acc, media) => {
                    return media.title.english
                      ? { ...acc, [media.title.english]: media.id }
                      : acc;
                  }, {})}
                  onChange={(mediaId: string) => {
                    signal.current.media = mediaId
                ...
                />
                {signal.current.media?.length ? (
                  <Select
                    required
                    label={i18n("role")}
                    list={CharacterRole}
                    // deno-lint-ignore no-non-null-assertion
                    defaultValue={signal.current.media![0].role}
                    onChange={(role: CharacterRole) => {
                      // deno-lint-ignore no-non-null-assertion
                      signal.current.media![0].role = role;
                      onCharacterUpdate();
                      forceUpdate();
                    }}
                  />
                ...
                <div className={"flex"}>
                  <div className={"flex grow"}>
                    <Star
                      ...
                      onClick={() => {
                        signal.current.popularity = getPopularity(1);
                        onCharacterUpdate();
                        forceUpdate();
                      }}
                    />
                    <Star
                      ...
                      onClick={() => {
                        signal.current.popularity = getPopularity(2);
                        onCharacterUpdate();
                        forceUpdate();
                      }}
                    />
                    <Star
                      ...
                      onClick={() => {
                        signal.current.popularity = getPopularity(3);
                        onCharacterUpdate();
                        forceUpdate();
                      }}
                    />
                    <Star
                      ...
                      onClick={() => {
                        signal.current.popularity = getPopularity(4);
                        onCharacterUpdate();
                        forceUpdate();
                      }}
                    />
                    <Star
                      ...
                      onClick={() => {
                        signal.current.popularity = getPopularity(5);
                        onCharacterUpdate();
                        forceUpdate();
                      }}
                    />
              ...
              <div className={"flex flex-row gap-2"}>
                <TextInput
                  ...
                  value={signal.current.age ?? ""}
                  onInput={(value) => {
                    signal.current.age = value || undefined;
                    onCharacterUpdate();
                  }}
                  key={`${signal.current.id}-age`}
                />
                ...
                <TextInput
                  ...
                  value={signal.current.gender ?? ""}
                  onInput={(value) => {
                    signal.current.gender = value || undefined;
                    onCharacterUpdate();
                  }}
                  key={`${signal.current.id}-gender`}
                />
              </div>
              ...
              <TextInput
                ...
                onInput={(value) => {
                  signal.current.description = value || undefined;
                  onCharacterUpdate();
                }}
              ...
              <div className={"flex flex-col gap-4"}>
                ...
                <div className={"flex flex-wrap gap-2"}>
                  {signal.current.name.alternative?.map((alias, i) => (
                    <div
                      className={
                    ...
                    >
                      ...
                      <Trash2
                        ...
                        onClick={() => {
                          // deno-lint-ignore no-non-null-assertion
                          signal.current.name.alternative!.splice(i, 1);
                          onCharacterUpdate();
                          forceUpdate();
                        }}
                      />
                  ...
                  {(signal.current.name.alternative?.length ?? 0) < 5 ? (
                    <div
                      className={
                        "flex items-center justify-center bg-embed rounded-[100vw] px-6 py-4 gap-2"
                      }
                    >
                      ...
                      <Plus
                        ...
                        onClick={() => {
                          if (!signal.current.name.alternative) {
                            signal.current.name.alternative = [];
                          }

                          const length = newAliasValue?.length || 0;

                          if (length >= 1 && length <= 128) {
                            signal.current.name.alternative.push(newAliasValue);
              ...
              <div className={"flex flex-col"}>
                ...
                <div className={"flex flex-col gap-2"}>
                  ...
                  {(signal.current.externalLinks?.length ?? 0) < 5 ? (
                    <button
                      onClick={() => {
